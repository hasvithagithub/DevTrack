...
const Repositories = () => {
  ...
  const getVisibilityColor = (vis) => {
    switch (vis) {
      case 'Public': return 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400';
      case 'Private': return 'bg-red-500/10 text-red-600 dark:text-red-400';
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 dark:text-slate-500 font-medium mt-1">
            Browse and manage software repositories hosted on your company servers.
          </p>
        </div>
      ...
      >
        {filteredRepos.length > 0 ? (
            ...
            >
              ...
              <div>
                <div className="flex items-start justify-between gap-4">
                  ...
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider ${getVisibilityColor(repo.visibility)}`}>
                    {repo.visibility}
                  </span>
                </div>
              ...
              <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 grid grid-cols-3 gap-2 text-center">
                <div className="flex flex-col">
                  <span className="text-[10px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Branches</span>
                  <span className="text-sm font-bold text-slate-700 dark:text-white mt-0.5">{repo.branchesCount}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[10px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Issues</span>
                  <span className="text-sm font-bold text-red-500 mt-0.5">{repo.openIssuesCount}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[10px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">PRs</span>
                  <span className="text-sm font-bold text-green-500 mt-0.5">{repo.openPRsCount}</span>
                </div>
              </div>
              ...
              <div className="mt-5 flex items-center justify-between">
                <span className="text-[10px] text-slate-400 dark:text-slate-500 font-medium flex items-center gap-1">
                  <FiClock /> updated {new Date(repo.lastUpdated).toLocaleDateString([], { month: 'short', day: 'numeric' })}
                </span>
          ...
          <div className="col-span-full py-12 text-center text-slate-400 dark:text-slate-500 border border-dashed border-slate-200 dark:border-slate-800 rounded-3xl bg-white dark:bg-[#1E293B]/20">
            ...
            <h3 className="text-base font-bold text-slate-700 dark:text-white">No repositories found</h3>
