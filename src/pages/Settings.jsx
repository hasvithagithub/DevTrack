...
import { useCurrentUser, useOrgInfo } from "../hooks/useDevTrackQueries";

const getStoredAuthProfile = () => {
  try {
    const stored = JSON.parse(localStorage.getItem("devtrack-auth") || "{}");
    return {
    ...
    return { name: "User", bio: "", email: "", location: "", avatar: "" };
  }
};
...
const Settings = () => {
  ...
  // Profile Forms
  const [profName, setProfName] = useState(storedProfile.name);
  const [profBio, setProfBio] = useState(storedProfile.bio);
  const [profEmail, setProfEmail] = useState(storedProfile.email);
  const [profLoc, setProfLoc] = useState(storedProfile.location);
  const [profAvatar, setProfAvatar] = useState(storedProfile.avatar);
  ...
  const [apiTokens, setApiTokens] = useState([
    {
      id: 1,
      name: "Local VSCode",
      token: "dt_pat_8f98...a8d1",
      created: "2026-06-12",
    },
    {
      id: 2,
      name: "CI/CD Pipeline runner",
      token: "dt_pat_2c7d...e9b4",
      created: "2026-06-25",
    },
  ]);
  ...
  useEffect(() => {
    ...
    const nextProfile = {
      name: currentUser.full_name || currentUser.login || storedProfile.name,
      bio: currentUser.description || currentUser.bio || storedProfile.bio,
      email: currentUser.email || storedProfile.email,
      location: currentUser.location || storedProfile.location,
      avatar: currentUser.avatar_url || storedProfile.avatar,
    };

    setProfName(nextProfile.name);
    setProfBio(nextProfile.bio);
    setProfEmail(nextProfile.email);
    setProfLoc(nextProfile.location);
    setProfAvatar(nextProfile.avatar);
  }, [
    currentUser,
    storedProfile.name,
    storedProfile.bio,
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-400 dark:text-slate-505 font-medium mt-1">
          Configure profile details, application preferences, visual themes, and
          API tokens.
        </p>
      </div>
            ...
            <form onSubmit={handleSaveProfile} className="space-y-6 max-w-xl">
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium">
                  Update account branding details visible to colleagues.
                </p>
              </div>
              ...
              <div className="flex items-center gap-4">
                <img
                  src={
                    profAvatar ||
                    `https://ui-avatars.com/api/?name=${encodeURIComponent(profName || "User")}&background=3b82f6&color=fff&size=128`
                  }
                  alt={profName || "User"}
                  className="w-16 h-16 rounded-full border object-cover"
                  onError={(e) => {
                    e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(profName || "User")}&background=3b82f6&color=fff&size=128`;
                  }}
                />
                <button
                  type="button"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-450 dark:text-slate-500 uppercase tracking-wider">
                    Full Name
                  </label>
                  <input
                    type="text"
          ...
          {activeTab === "preferences" && (
            <div className="space-y-6 max-w-xl">
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium">
                  Control notifications rules and dark mode defaults.
                </p>
              </div>
              ...
              <div className="space-y-3.5 pt-4 border-t border-slate-105 dark:border-slate-850">
                ...
                <div className="space-y-3">
                  <label className="flex items-start cursor-pointer">
                    ...
                    <div className="ml-3 flex flex-col">
                      ...
                      <span className="text-[10px] text-slate-400 font-medium">
                        Receive summaries when patches are pushed.
                      </span>
                    </div>
                  </label>
                  ...
                  <label className="flex items-start cursor-pointer">
                    ...
                    />
                    <div className="ml-3 flex flex-col">
                      <span className="text-xs font-bold text-slate-755 dark:text-slate-200">
                        Email on Pull Requests
                      </span>
                      <span className="text-[10px] text-slate-400 font-medium">
                        Receive alerts when merges request reviews.
                      </span>
                    </div>
                  </label>
                  ...
                  <label className="flex items-start cursor-pointer">
                    ...
                    />
                    <div className="ml-3 flex flex-col">
                      <span className="text-xs font-bold text-slate-760 dark:text-slate-200">
                        Email on Assigned Issues
                      </span>
                      <span className="text-[10px] text-slate-400 font-medium">
                        Receive alerts when bugs are assigned to you.
                      </span>
                    </div>
                  </label>
                </div>
            ...
            <div className="space-y-6 max-w-xl">
              ...
              <div className="space-y-3.5">
                ...
                <div className="p-4 border border-dashed border-slate-200 dark:border-slate-800 rounded-2xl bg-slate-50/50 dark:bg-slate-900/10 text-center">
                  <p className="text-xs text-slate-450 dark:text-slate-500 font-medium mb-3">
                    Add SSH public keys to authenticate Git operations from
                    local terminal.
                  </p>
                  <button
                    type="button"
                ...
                <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900/50 border border-slate-150 dark:border-slate-850 rounded-2xl">
                  <div>
                    ...
                    <span className="text-[10px] text-slate-400 block mt-0.5">
                      Use Google Authenticator to secure sign-in.
                    </span>
                  </div>
                  <button
                    type="button"
                    ...
                    onClick={() =>
                      alert("Two-factor config workflow simulated.")
                    }
                  >
                    Enable 2FA
                  </button>
          ...
          {activeTab === "api" && (
            <div className="space-y-6">
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium">
                  Generate Personal Access Tokens to query Gitea web endpoints.
                </p>
              </div>
              ...
              <form
                ...
                className="p-4 border border-slate-150 dark:border-slate-850 bg-slate-50/50 dark:bg-slate-900/20 rounded-2xl max-w-xl space-y-3"
              >
                  ...
                  <table className="w-full text-left text-xs border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50 font-medium text-slate-500">
                      {apiTokens.map((t) => (
                        <tr
                          key={t.id}
                          className="hover:bg-slate-50/30 dark:hover:bg-slate-800/10"
                        >
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-450 dark:text-slate-550 mt-1">
                    Enterprise dashboard for self-hosted Git services.
                  </p>
                </div>
              </div>
