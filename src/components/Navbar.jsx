...
const Navbar = ({ setIsMobileOpen, isCollapsed }) => {
  ...
  const getBreadcrumbs = () => {
    ...
    paths.forEach((p, idx) => {
      ...
      if (idx === 1 && paths[0] === 'repositories') {
        const repo = repositories.find(r => r.name.toLowerCase() === p.toLowerCase());
        if (repo) label = repo.name;
      } else if (idx === 1 && paths[0] === 'developers') {
        const dev = developers.find(d => d.username.toLowerCase() === p.toLowerCase());
  ...
  useEffect(() => {
    ...
    const matchedRepos = repositories.filter(r => r.name.toLowerCase().includes(query) || r.description.toLowerCase().includes(query)).slice(0, 3);
    const matchedDevs = developers.filter(d => d.name.toLowerCase().includes(query) || d.username.toLowerCase().includes(query)).slice(0, 3);
    const matchedCommits = commits.filter(c => c.message.toLowerCase().includes(query) || c.hash.includes(query)).slice(0, 3);
    const matchedIssues = issues.filter(i => i.title.toLowerCase().includes(query)).slice(0, 3);
    const matchedPRs = pullRequests.filter(p => p.title.toLowerCase().includes(query)).slice(0, 3);
    const matchedBranches = branches.filter(b => b.name.toLowerCase().includes(query)).slice(0, 3);

    const hasResults = matchedRepos.length > 0 || matchedDevs.length > 0 || matchedCommits.length > 0 || matchedIssues.length > 0 || matchedPRs.length > 0 || matchedBranches.length > 0;
    
    setSearchResults(hasResults ? {
      repos: matchedRepos,
      devs: matchedDevs,
  ...
  return (
    <nav className="h-16 px-4 md:px-6 bg-white dark:bg-[#1E293B] border-b border-slate-200 dark:border-slate-800 flex items-center justify-between sticky top-0 z-20 transition-colors duration-150 shadow-sm">
        ...
        <div className="hidden sm:flex items-center gap-1 text-sm font-medium">
          ...
          {getBreadcrumbs().map((bc, idx) => (
            <React.Fragment key={idx}>
              {idx > 0 && <FiChevronRight className="text-slate-300 dark:text-slate-600" />}
              {idx === getBreadcrumbs().length - 1 ? (
                <span className="text-slate-700 dark:text-slate-200 font-semibold truncate max-w-[120px] md:max-w-xs">{bc.label}</span>
              ) : (
                <Link 
                  to={bc.path}
      ...
      <div className="flex items-center gap-3 md:gap-5">
            ...
            <div className="absolute right-0 mt-2 w-72 sm:w-96 md:w-[480px] max-h-96 overflow-y-auto bg-white dark:bg-[#1E293B] border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl z-50 p-2 scrollbar">
              {searchResults ? (
                  ...
                  {searchResults.repos?.length > 0 && (
                    <div>
                      ...
                      {searchResults.repos.map(r => (
                        <button
                          key={r.id}
                          onClick={() => handleSearchResultClick(`/repositories/${r.name}`)}
                        ...
                        >
                          <span className="text-sm font-semibold text-blue-600 dark:text-blue-400">{r.name}</span>
                          <span className="text-xs text-slate-400 dark:text-slate-500 line-clamp-1">{r.description}</span>
                        </button>
                      ))}
                    </div>
                  )}
                  ...
                  {searchResults.devs?.length > 0 && (
                    <div>
                      ...
                      {searchResults.devs.map(d => (
                        <button
                          key={d.id}
                          onClick={() => handleSearchResultClick(`/developers/${d.username}`)}
                        ...
                        >
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs text-slate-400 dark:text-slate-500">@{d.username}</span>
                          </div>
                        </button>
                      ))}
                    </div>
                  ...
                  {searchResults.commits?.length > 0 && (
                    <div>
                      ...
                      {searchResults.commits.map(c => (
                        <button
                          key={c.hash}
                          onClick={() => handleSearchResultClick(`/commits?search=${c.hash}`)}
                        ...
                        >
                          <span className="text-sm font-medium text-slate-700 dark:text-slate-200 line-clamp-1">{c.message}</span>
                          <div className="flex items-center gap-2 text-xs text-slate-400 dark:text-slate-500 mt-0.5">
                            <span className="font-mono text-blue-500">{c.shortHash}</span>
                            <span>•</span>
                  ...
                  {searchResults.issues?.length > 0 && (
                    <div>
                      ...
                      {searchResults.issues.map(i => (
                        <button
                          key={i.id}
                          onClick={() => handleSearchResultClick(`/issues?search=${i.id}`)}
                          className="w-full text-left px-3 py-1.5 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 flex flex-col transition-colors"
                        >
                          <span className="text-sm font-medium text-slate-700 dark:text-slate-200 line-clamp-1">{i.title}</span>
                          <span className="text-xs text-slate-400 dark:text-slate-500">{i.repository}</span>
                        </button>
                      ))}
                    </div>
                  )}
                  ...
                  {searchResults.prs?.length > 0 && (
                    <div>
                      ...
                      {searchResults.prs.map(p => (
                        <button
                          key={p.id}
                          onClick={() => handleSearchResultClick(`/pull-requests?search=${p.id}`)}
                          className="w-full text-left px-3 py-1.5 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 flex flex-col transition-colors"
                        >
                          <span className="text-sm font-medium text-slate-700 dark:text-slate-200 line-clamp-1">{p.title}</span>
                          <span className="text-xs text-slate-400 dark:text-slate-500">{p.repository}</span>
                        </button>
                      ))}
                    </div>
                  )}
                  ...
                  {searchResults.branches?.length > 0 && (
                    <div>
                      ...
                      {searchResults.branches.map(b => (
                        <button
                          key={b.id}
                          onClick={() => handleSearchResultClick(`/branches?search=${b.name}`)}
                          className="w-full text-left px-3 py-1.5 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 flex flex-col transition-colors"
                        >
                          <span className="text-sm font-medium text-slate-700 dark:text-slate-200 font-mono">{b.name}</span>
                          <span className="text-xs text-slate-400 dark:text-slate-500">{b.repository}</span>
                        </button>
                      ))}
                    </div>
                  )}
                ...
                <div className="p-4 text-center text-sm text-slate-400 dark:text-slate-500">
                  No matching results found for "<span className="font-semibold">{searchQuery}</span>"
                </div>
              )}
            </div>
          )}
                ...
                {notificationsList.length > 0 ? (
                  notificationsList.map(n => (
                    ...
                    >
                      ...
                      <div className="flex flex-col">
                        <span className="text-xs text-slate-600 dark:text-slate-300 font-medium line-clamp-2">{n.message}</span>
        ...
        <Link to="/settings" className="flex items-center gap-2 hover:opacity-85 transition-opacity">
          <img
          ...
          <div className="hidden lg:flex flex-col text-left">
            <span className="text-xs font-bold text-slate-700 dark:text-slate-200 leading-tight">Sarah Wilson</span>
            <span className="text-[10px] text-slate-400 dark:text-slate-500">Administrator</span>
          </div>
        </Link>
