...
app.post("/api/auth/login", async (req, res) => {
  ...
  try {
    ...
    res
      .status(401)
      .json({ error: "Invalid token or Gitea server unreachable" });
  }
});
...
app.use("/api/gitea", async (req, res) => {
  const token =
    req.cookies.gitea_token || req.headers.authorization?.split(" ")[1];
  ...
  try {
    const giteaPath = req.path.replace(/^\/gitea/, "");
    const url = `${GITEA_URL}/api/v1${giteaPath}`;
...
app.post("/api/attendance", (req, res) => {
  ...
  db.run(
    `INSERT INTO attendance_logs (username, type) VALUES (?, ?)`,
    [username, type],
    function (err) {
      if (err) {
        return res.status(500).json({ error: err.message });
